...
contract('Escrow', (accounts) => {
  ...
  it('Should NOT release funds if full amount has not been released', async () => {
    assertError(
  ...
  it('Should release funds', async () => {
    ...
    const balanceRecipientAfer = web3.utils.toBN(await web3.eth.getBalance(recipient));
    assert(balanceRecipientAfer.sub(balanceRecipientBefore).toNumber() === 1000);
  });
